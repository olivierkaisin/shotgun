...
// Screenshot pages
app.get('/:type(iphone|desktop-large|desktop-small|desktop-extra-large)', function (req, res) {
  ...
  webshot(req.query.url, shotPath, options, function (err) {
    ...
    if (err) {
      return logger.error({url: req.query.url, component: 'webshot'}, err);
    }
    ...
    gm(readStream)
      ...
      .stream(function (err, stdout, stderr) {
        if (err) {
          return logger.error({url: req.query.url, component: 'resizer'}, err);
        }
